...
// Configuration
const N8N_WEBHOOK_URL = 'https://n8n.srv1081620.hstgr.cloud/webhook/chat-web';
const N8N_CREATE_RECIPE_WEBHOOK = 'https://n8n.srv1081620.hstgr.cloud/webhook/create-recepie';
const N8N_ACCEPT_RECIPE_WEBHOOK = 'https://n8n.srv1081620.hstgr.cloud/webhook/accept-recepie';
const N8N_MODIFY_RECIPE_WEBHOOK = 'https://n8n.srv1081620.hstgr.cloud/webhook/modifie-recepie';
...
app.post('/api/create-recipe', async (req, res) => {
  try {
    ...
    console.log(`Creating recipe via n8n: ${title}`);

    // Envoyer les données au webhook n8n
    const n8nResponse = await axios.post(N8N_CREATE_RECIPE_WEBHOOK, {
      title,
      description,
      ingredients,
      recipe,
    ...
    console.log('n8n recipe response received:', n8nResponse.data);

    // Renvoyer la réponse de n8n au frontend
    res.json({
      success: true,
      ...n8nResponse.data
    });
    ...
    console.error('Error creating recipe via n8n:', error.response?.data || error.message);

    let errorMessage = 'Failed to create recipe via n8n';
...
// Endpoint pour accepter une recette via n8n (v3.10.2)
app.post('/api/accept-recipe', async (req, res) => {
  try {
    const recipeData = req.body;

    console.log('Accepting recipe via n8n:', recipeData.title);

    // Convertir ingredients et recipe en JSON string pour Airtable
    const dataToSend = {
      title: recipeData.title,
      description: recipeData.description,
      ingredients: JSON.stringify(recipeData.ingredients), // Array → JSON string
      recipe: JSON.stringify(recipeData.recipe), // Array → JSON string
      calories: recipeData.calories,
      proteines: recipeData.proteines,
      glucides: recipeData.glucides,
      lipides: recipeData.lipides,
      timestamp: new Date().toISOString(),
      status: 'accepted'
    };

    console.log('Complete payload to n8n (with stringified arrays):', JSON.stringify(dataToSend));

    const n8nResponse = await axios.post(N8N_ACCEPT_RECIPE_WEBHOOK, dataToSend, {
      timeout: 30000 // 30 secondes
    });

    console.log('n8n accept response received:', n8nResponse.data);

    // Renvoyer succès au frontend
    res.json({
      success: true,
      message: 'Recette acceptée et enregistrée !',
      ...n8nResponse.data
    });

  } catch (error) {
    console.error('Error accepting recipe via n8n:', error.response?.data || error.message);

    let errorMessage = 'Failed to accept recipe via n8n';
...
// Endpoint pour modifier une recette via n8n (v3.10.3)
app.post('/api/modify-recipe', async (req, res) => {
  try {
    const recipeData = req.body;

    console.log('Modifying recipe via n8n:', recipeData.title);

    // Envoyer toutes les infos au webhook n8n
    const dataToSend = {
      remark: recipeData.remark || '', // Remarque de modification
      title: recipeData.title,
      description: recipeData.description,
      ingredients: recipeData.ingredients, // Format texte pour modification
      recipe: recipeData.recipe, // Format texte pour modification
      calories: recipeData.calories || 0,
      proteines: recipeData.proteines || 0,
      glucides: recipeData.glucides || 0,
      lipides: recipeData.lipides || 0,
      timestamp: new Date().toISOString()
    };

    console.log('Sending complete modification request to n8n:', dataToSend);

    const n8nResponse = await axios.post(N8N_MODIFY_RECIPE_WEBHOOK, dataToSend, {
      timeout: 120000 // 2 minutes pour laisser le temps à n8n de traiter (agents IA)
    });

    console.log('n8n modify response received:', n8nResponse.data);
    ...
    console.error('Error modifying recipe via n8n:', error.response?.data || error.message);

    let errorMessage = 'Failed to modify recipe via n8n';
...
app.get('/api/recipes', async (req, res) => {
  try {
    ...
    const recipes = response.data.records.map(record => ({
      ...
      recipe: record.fields.Recette || [], // v3.6: Recipe steps as JSON array
      calories: record.fields.Calories || record.fields['Calories (totales)'] || 0, // v3.6: Updated field name
...
app.get('/api/planning', async (req, res) => {
  try {
    ...
    const planning = response.data.records.map(record => ({
      ...
      servings: record.fields['Nombre de personnes'] || 2 // v3.7: Return servings
    }));
...
app.post('/api/planning', async (req, res) => {
  try {
    const { day, date, meal, recipeId, week, year, servings } = req.body;

    console.log(`Adding recipe ${recipeId} to planning: ${day} ${meal} (Week ${week}, Year ${year}, ${servings} personnes)`);
    ...
    const response = await axios.post(
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/Plannings%20Hebdomadaires`,
      {
        fields: {
          ...
          'Semaine': week,
          'Annee': year,
          'Nombre de personnes': servings || 2 // v3.7: Default to 2 if not provided
        }
      },
      {
        headers: {
...
// PATCH : Mettre à jour un repas du planning (v3.7 - pour servings)
app.patch('/api/planning/:id', async (req, res) => {
  try {
    const recordId = req.params.id;
    const { servings } = req.body;

    console.log(`Updating planning record ${recordId}: servings = ${servings}`);
    ...
    res.json({
      ...
      record: {
        id: response.data.id,
        servings: response.data.fields['Nombre de personnes']
      }
    });

  } catch (error) {
    console.error('Error updating planning:', error.response?.data || error.message);
    res.status(500).json({
